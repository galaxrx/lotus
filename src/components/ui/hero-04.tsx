...
export function Hero04({
  ...
  // NOTE: adapted for Tailwind v3 — the original used v4 `mask-radial-*`/`aspect-2/3`
  // utilities. We reproduce the top-anchored radial fade with an inline mask so the
  // wash reads as a subtle accent (not a full-bleed image) in light mode too.
  const washMask =
    "radial-gradient(75% 100% at 50% 0%, black 40%, transparent 72%)"
  const backgroundElement = washImage && (
    <div
      aria-hidden
      className="pointer-events-none absolute inset-0 z-0 opacity-40 blur-xl dark:opacity-[0.06]"

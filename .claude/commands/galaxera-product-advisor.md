---
description: Run the Galaxera Product Advisor (Codex read-only product/UX/UI review gate)
argument-hint: "[pre-implementation|post-implementation] [feature or notes]"
---

Invoke the **galaxera-product-advisor** skill now and follow it exactly.
...
Steps:
1. Load the skill at `.claude/skills/galaxera-product-advisor/SKILL.md` and obey its workflow.
2. Assemble the focused context package the skill describes (never include secrets,
   `.env`, or private data).
3. Run `node .claude/skills/galaxera-product-advisor/scripts/consult-codex.mjs`
   with the chosen `--mode` and the context, and save the returned review so I can read it.
